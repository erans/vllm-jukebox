...
func TestLifecycle_ComfyUIAccepted(t *testing.T) {
	cfg, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
`)
	...
	m := cfg.Models["sdxl"]
	if m.EffectiveLifecycle() != config.LifecycleComfyUI {
		t.Fatalf("expected comfyui lifecycle, got %q", m.EffectiveLifecycle())
	}
	if m.Host != "comfyui" || m.Port != 8188 {
		t.Fatalf("expected host=comfyui port=8188, got host=%q port=%d", m.Host, m.Port)
	}
}

func TestLifecycle_ComfyUIRequiresHost(t *testing.T) {
	// ComfyUI does NOT default host (unlike external, which falls back
	// to 127.0.0.1) — explicit host is required, since ComfyUI is
	// almost always a sibling container reachable by service-name.
	_, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
func TestLifecycle_ComfyUIRequiresPort(t *testing.T) {
	_, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
func TestLifecycle_ComfyUIRejectsAlias(t *testing.T) {
	_, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
`)
	if err == nil || !strings.Contains(err.Error(), "alias") {
		t.Fatalf("expected comfyui+alias rejection, got: %v", err)
	}
}

func TestLifecycle_ComfyUIRequiresSchedulerMode(t *testing.T) {
	_, err := loadFromYAML(t, `
vllm:
  port: 8000
models:
...
func TestLifecycle_ComfyUIOmitsPathRequirement(t *testing.T) {
	// Like external, comfyui doesn't need 'path' — the model loading
	// happens inside ComfyUI's own workflows.
	if _, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
models:
  sdxl:
    ...
    min_free_mem_mb_per_gpu: 4000
`); err != nil {
		t.Fatalf("expected comfyui without path to parse, got: %v", err)
	}
}

func TestLifecycle_ComfyUIWithLlamaCppRejected(t *testing.T) {
	_, err := loadFromYAML(t, `
scheduler:
  port_range_start: 8100
  port_range_end: 8109
...
`)
	if err == nil {
		t.Fatalf("expected comfyui+llama_cpp rejection, got nil")
	}
}
