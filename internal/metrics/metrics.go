...
var (
	...
	// AdmissionStateReconciliationTotal bumps each time an admission-state
	// reconciliation fires, labeled by model and the reason the
	// reconciliation was triggered (e.g. reprobe_cleared_warming when a
	// post-wake reprobe clears a stale warming-up admission state). Use the
	// reason label to attribute reconciliations to their source path.
	AdmissionStateReconciliationTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "jukebox_admission_state_reconciliation_total",
			Help: "Total admission-state reconciliations by model and reason (e.g. reprobe_cleared_warming).",
		},
		[]string{"model", "reason"},
	)
...
func init() {
	prometheus.MustRegister(
		...
		AdmissionStateReconciliationTotal,
