...
func (f *fakeVLLMServer) Host() string  { h, _, _ := net.SplitHostPort(strings.TrimPrefix(f.URL(), "http://")); return h }
func (f *fakeVLLMServer) Port() int     { _, p, _ := net.SplitHostPort(strings.TrimPrefix(f.URL(), "http://")); n, _ := strconv.Atoi(p); return n }
