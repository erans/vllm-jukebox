...
// SleepForEviction. The controller RELEASES its admission mutex across
// this call (so the 60s+ drain+sleep+settle stays off the lock — see
// RequestWake's evictor dispatch) and RE-ACQUIRES it to apply the
// post-evict accounting itself. A callback would therefore not deadlock
// on the lock, but it WOULD double-book: the controller already updates
// its own budget bookkeeping after SleepForEviction returns, so a
// callback that also mutates budgets corrupts the accounting.
//
...
// contract as SleepForEviction applies: the controller RELEASES a.mu
// across this call and RE-ACQUIRES it afterward, so implementations MUST
// NOT call back into AdmissionController methods — not because of a
// held-lock deadlock, but because the controller applies the post-stop
// accounting itself and a callback would double-book.
//
...
// markStartedAwakeLocked is the non-sleep-mode analogue of
// markStartedLocked: a Stopped peer's container has come back HEALTHY but
// the model has no sleep_mode, so it is fully resident (awake) rather than
// slept-L1. Caller must hold a.mu. Books the FULL expected awake VRAM (not
// the L1 residual) and snapshots BookedExpectedVRAMMB so a subsequent
// NotifyStopped reversal stays balanced.
//
// Snapshot discipline mirrors RequestWake's awake-booking block: book from
// the LIVE config (m.GPUs / m.ExpectedOn) and record the snapshot for
// later reversal.
func (a *AdmissionController) markStartedAwakeLocked(m *modelAdmissionState) {
	if m.State != admissionStopped {
		return
	}
	booked := make(map[int]int, len(m.GPUs))
...
// NotifyStartedAwake reconciles a Stopped, non-sleep-mode model whose
// container has come back HEALTHY directly to the awake (fully-resident)
// state — booking full expected VRAM, NOT the L1 residual that
// NotifyStarted books. Used by ReprobeStoppedExternal for an external
// model declared sleep_mode:false: such a model that comes back is just
// "up", never "asleep" — it has no /wake_up endpoint to call.
//
// Safe to call for untracked models or for models that aren't currently
// Stopped (no-op).
func (a *AdmissionController) NotifyStartedAwake(name string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	m, ok := a.models[name]
	if !ok {
...
// NotifyAdopted ratifies an adopted peer's physical state into the admission books
// WITHOUT running the budget gate. Called once by RegisterExternalInstances after
// each peer probe. Unlike RequestWake, this unconditionally records reality — the
// peer is already running (awake=true) or already sleeping (awake=false) before
// jukebox started. Idempotent: no-op if model is not in admissionUnknown state
// (covers pinned peers already set to admissionAwake by NewAdmissionController).
//
// CRITICAL: this method MUST set all Booked* snapshot fields so subsequent
// NotifySleep/NotifyStopped reversals work. Adversarial review (a6a3f53b) flagged
// 3 HIGH-severity phantom-leak scenarios that would occur without these fields.
func (a *AdmissionController) NotifyAdopted(name string, awake bool) {
	if a == nil {
		return
	}
	a.mu.Lock()
	defer a.mu.Unlock()
	m, ok := a.models[name]
	if !ok || m.State != admissionUnknown {
		return
	}
	if awake {
		booked := make(map[int]int, len(m.GPUs))
...
//  2. Build the candidate pool, partitioned into three tiers:
//     a. NON-group: out-of-group, non-pinned, non-critical awake peers
//     sharing ≥1 GPU with `m`. Cheapest to evict.
//     b. SAME-group: awake peers in m's swap_group (any priority/pinned).
//     Operator declared the swap relationship — eviction is by-design.
//     c. CROSS-group swap-evictable: out-of-group pinned/critical awake
//     peers whose own SwapGroup is non-empty. The operator declared
//     these as evict-eligible at the group level. This was the gap
//     introduced when admission landed: a non-pinned non-grouped
//     requester (e.g. vllm-vision) was rejected before reaching the
//     cross-group eviction code path because pickVictims excluded ALL
//     out-of-group pinned peers, even those in evict-eligible groups.
//  3. Sort each tier (priority bucket asc, LRU asc), then walk in tier
//     order: 1 → 2 → 3. Last tier is most disruptive (evicts an
//     operator-pinned peer to make room for a non-grouped requester),
//     so it is only dipped into when cheaper tiers cannot satisfy.
//  4. For each candidate, pick if it reduces shortfall on any still-short
//     GPU. Subtract its per-GPU freed VRAM. Stop when all target GPUs
//     have shortfall ≤ 0.
//  5. If shortfall remains after exhausting candidates → return error.
//
// Pinned peers WITHOUT a swap_group are still untouchable on every tier
// — those are the "true unevictable" pinned models (e.g. a pinned model
// the operator never declared as swap-eligible).
//
...
func (a *AdmissionController) pickVictims(m *modelAdmissionState, needed map[int]int) ([]*modelAdmissionState, error) {
	...
	//   tier 1 — nonGroup: out-of-group, non-pinned, non-critical peers.
	//            Cheapest to evict (operator did not flag them as "leave
	//            alone"). Try first.
	//   tier 2 — sameGroup: peers in m's swap-group (any priority/pinned),
	//            because the operator declared the swap relationship.
	//   tier 3 — crossGroupSwapEvictable: out-of-group pinned/critical
	//            peers whose own SwapGroup is non-empty. The operator
	//            declared their group as evict-eligible (members may be
	//            slept/stopped to make room for an in-group swap), and
	//            evict_action is sleep|stop (the only valid values today),
	//            so it is reasonable to evict them to make room for a
	//            cross-group requester whose GPUs they are blocking.
	//            This is the gap admissionCanAdmit closed for the
	//            "vllm-vision wakes onto a GPU pinned by vllm-main, where
	//            vllm-main is in gpu-4-evict swap-group" topology.
	//            Pinned-non-swap-group peers are STILL untouchable.
	nonGroup := []*modelAdmissionState{}
	sameGroup := []*modelAdmissionState{}
	crossGroupSwapEvictable := []*modelAdmissionState{}
	for name, peer := range a.models {
		...
		if peer.Pinned || peer.Priority == config.PriorityCritical {
			// Pinned/critical out-of-group peers are NORMALLY sacred,
			// EXCEPT when the operator opted them into a swap-group that
			// the cross-group eviction policy honors. A non-empty
			// peer.SwapGroup tells us the operator accepts the peer can
			// be slept/stopped to free its GPU set; both EvictAction
			// values (sleep, stop) qualify (they describe HOW to evict,
			// not WHETHER to evict). Without this branch, vllm-vision /
			// vllm-ocr (no swap-group) cannot wake while vllm-main
			// (pinned + gpu-4-evict swap-group) holds GPU 1, even though
			// vllm-main is by-design swap-eligible — admission would
			// reject before evictCrossGroupGPUContendersLocked or our
			// own evictor got a chance to run.
			if peer.SwapGroup != "" && (peer.EvictAction == "" ||
				peer.EvictAction == config.EvictActionSleep ||
				peer.EvictAction == config.EvictActionStop) {
				crossGroupSwapEvictable = append(crossGroupSwapEvictable, peer)
			}
			continue
		}
	...
	tierSort(nonGroup)
	tierSort(sameGroup)
	tierSort(crossGroupSwapEvictable)

	// Walk tier 1 (non-group) first, then tier 2 (same-group), then
	// tier 3 (cross-group swap-evictable pinned/critical). Last tier is
	// most disruptive — only dipped into when the cheaper tiers cannot
	// satisfy the shortfall. Concatenation keeps the pick loop simple.
	candidates := append(nonGroup, sameGroup...)
	candidates = append(candidates, crossGroupSwapEvictable...)
...
// effectivePriorityRank returns the sort key for a model's EFFECTIVE
// priority, applying the same pinned→critical promotion the admission
// controller uses (a pinned model is treated as critical regardless of
// its declared Priority — see markStartedLocked / refresh). Used by the
// cross-group eviction guard in sleep.go to decide whether a resident
// peer outranks (and is therefore protected from) a cold-load target.
func effectivePriorityRank(m config.ModelConfig) int {
	if m.Pinned != nil && *m.Pinned {
		return priorityRank(config.PriorityCritical)
	}
	return priorityRank(m.EffectivePriority())
}
