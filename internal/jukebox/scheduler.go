...
func (s *Scheduler) Status() Status {
	...
	// Bug #4 / MED#4: populate CurrentModel so /status reflects what is
	// actually serving — WITHOUT ever naming a down model. In scheduler
	// mode there is no single swapped-in model (the coordinator's
	// CurrentModel field, which the swap-mode path sets, is meaningless
	// here), so /status historically reported current_model:"" even while
	// one or more instances served 200s. Report the most-recently-USED
	// Ready instance as the representative current model — the one a
	// consumer would observe as "the model jukebox is serving right now".
	// Falls back to the most-recently-used Sleeping instance (warm-on-
	// demand) only when nothing is Ready. A Stopped/Stopping/Starting/
	// etc. instance is NEVER reported: naming a Stopped model would make
	// /status lie "serving vllm-main" while it is actually down. When no
	// instance is Ready or Sleeping, the field stays "" (honest blank).
	// Ties broken by name for determinism.
	var currentModel string
	var bestUsed time.Time
	var bestReady bool
	for _, inst := range s.instances {
		isReady := inst.state == StateReady && !inst.draining && inst.mgr != nil && inst.mgr.CurrentPID() != 0
		// Only Ready or Sleeping instances are eligible to be named as the
		// current model. Everything else (Stopped, Stopping, Starting,
		// Idle, draining-Ready) is excluded so we never report a model that
		// is not actually serving or wakeable-on-demand.
		eligible := isReady || inst.state == StateSleeping
		if !eligible {
			continue
		}
		// Prefer a Ready instance over a (non-Ready, eligible) Sleeping one.
		// Among the same readiness class, prefer the more-recently-used;
		// break ties by lexicographically smaller name for reproducible
		// output.
		better := false
		switch {
		case isReady && !bestReady:
			better = true
		case isReady == bestReady:
			if inst.lastUsedAt.After(bestUsed) {
				better = true
			} else if inst.lastUsedAt.Equal(bestUsed) && (currentModel == "" || inst.model < currentModel) {
				better = true
			}
		}
		if better {
			currentModel = inst.model
			bestUsed = inst.lastUsedAt
			bestReady = isReady
		}
	}
