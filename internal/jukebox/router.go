...
type ColdLoadAware interface {
	...
	// ReprobeStoppedExternal is the Bug #3 on-request health re-probe.
	// When IsModelColdLoading would 503 a request with "warming_up", the
	// handler calls this first: if `name` is an external-lifecycle model
	// that admission still believes is Stopped but whose container is in
	// fact healthy (/health=200 — e.g. it was recreated out-of-band), it
	// reconciles admission (Stopped→Sleeping) and returns true so the
	// handler routes through the normal wake path instead of returning a
	// stale 503. Returns false (and takes no action) for every other
	// case, so the async-503 + KickColdLoad contract is unchanged when
	// the container really is down. Bounded probe; safe to call on the
	// hot path.
	ReprobeStoppedExternal(ctx context.Context, name string) bool
}
