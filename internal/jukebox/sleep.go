...
// evictCrossGroupGPUContendersLocked evicts admission-tracked peers from
// DIFFERENT swap-groups (or no swap-group) that overlap GPU sets with
// the cold-loading target. The existing evictPeersForColdLoadLocked only
...
//   - peer != target
//   - peer.SwapGroup != target.SwapGroup (anything in the same group is
//     handled by evictPeersForColdLoadLocked; this helper is the
//     CROSS-group complement)
...
func (s *Scheduler) evictCrossGroupGPUContendersLocked(ctx context.Context, name string, modelCfg config.ModelConfig) (slept, stopped []string) {
	...
	for peerName, peerCfg := range s.cfg.Models {
		...
		// SAME swap-group → handled by evictPeersForColdLoadLocked. This
		// helper is the cross-group complement only.
		if peerCfg.SwapGroup != "" && peerCfg.SwapGroup == modelCfg.SwapGroup {
			continue
		}
		if !peerCfg.AdmissionEnabled() {
	...
	for _, c := range contenders {
		...
		if c.canSleep {
			...
			peerCfg, ok := liveModelCfg(s.cfg, c.name)
			if !ok {
				peerCfg = c.cfg
			}
