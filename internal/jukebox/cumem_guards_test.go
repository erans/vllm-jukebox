...
// per-model overrides for the cumem-drain + wake-settle knobs. The
// MinTimeSinceWakeSeconds gate is set to -1 (disabled) so existing
// guard A/B tests are not perturbed by the rapid-cycle anti-race
// gate's default-on (5s) behavior. Tests that exercise the rapid-
// cycle gate use minTimeSinceWakeTestCfg instead.
	...
	cfg := &config.Config{
		...
		Models: map[string]config.ModelConfig{
			modelName: {
				...
				MinTimeSinceWakeSeconds:  -1, // disable rapid-cycle gate for guard A/B isolation
			},
		},
	}
	return cfg
