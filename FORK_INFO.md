# Fork tracking — intarweb/vllm-jukebox

This is a soft fork of [`erans/vllm-jukebox`](https://github.com/erans/vllm-jukebox). We track upstream HEAD and publish a HEAD-tracked image to GHCR with any open intarweb→upstream PRs cherry-picked on top.
...
| Property | Value |
|---|---|
| Upstream | [`erans/vllm-jukebox`](https://github.com/erans/vllm-jukebox) |
...
| Property | Value |
|---|---|
| Image | `ghcr.io/intarweb/vllm-jukebox` |
...
services:
  vllm-jukebox:
    image: ghcr.io/intarweb/vllm-jukebox:latest
```

Pin to a specific build for reproducibility:

```yaml
    image: ghcr.io/intarweb/vllm-jukebox:sha-<short>
```
...
```bash
gh workflow run "Sync from upstream + auto-regen intarweb-dev" --repo intarweb/vllm-jukebox
```

**Force a fresh build of `:latest` without an upstream change:**
```bash
gh workflow run "Build from source → GHCR" --repo intarweb/vllm-jukebox --ref intarweb-dev
```
...
gh pr create --repo erans/vllm-jukebox --base main --head intarweb:feat/my-thing
